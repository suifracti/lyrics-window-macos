...
## 2026-08-08 当前真实歌曲覆盖复核

本次复核只读取当前数据库的歌曲元数据，并通过仓库内现有 Provider 做单次查询；没有持久化响应，也没有把歌曲标题、平台 ID、歌词正文或本地数据库路径写入仓库。

- 当前数据库共有 177 首带 Spotify ID 的歌曲，其中 27 首没有任何同步歌词版本。
- 更早的 AMLL 精确 ID 审计基线是 27 首缺口中 0 首命中；AMLL 仍适合零歧义精确读取，但不能解决这组冷门歌曲覆盖。
- 网易云实验源：3 首同步直接命中、6 首同步候选、18 首无匹配；无纯文本命中、超时或请求错误。
- QQ 音乐实验源：14 首同步直接命中、3 首同步候选、4 首纯文本直接命中、6 首无匹配；无超时或请求错误。
- 这些数字是 Provider 原始结果，不等于全部会被自动采用；候选仍必须经过 `LyricsSafeMatcher` 和用户确认。不同来源之间也可能命中同一首歌，因此不能把数字直接相加当作唯一覆盖数。

### 当前决定

1. 先保留并加固现有 QQ、网易实现，不立即新增酷狗。QQ 已覆盖这组缺口的大部分，新增来源的边际收益应在安全匹配、翻译层与版本选择体验完善后再测。
2. 两个实验源已经接入统一 `LyricsSearchManager`，只在“扩展免费实验模式”中运行；标准模式继续保持 Local、SQLite、AMLL、LRCLIB 的稳定边界。
3. 网络 Provider 采用本地优先、单源超时、同一查询变体并发、按配置顺序归并。完成顺序不能改变来源优先级，一个来源失败也不能取消其他来源。
4. TaskbarLyrics 的路由、候选重试思路可作为 MIT 参考，但其酷狗路径依赖 Lyricify helper 与 KRC 解密链路；Mineradio 为 GPL-3.0。若后续增加酷狗，应独立实现公开请求/响应协议并单独完成许可与失效风险审计，不复制 GPL 实现或私有凭据。
