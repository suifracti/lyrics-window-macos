...
FORMAL_DB="$HOME/Library/Application Support/SpotifyLyrics/SpotifyLyrics.sqlite3"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

if [[ ! -f "$FORMAL_DB" ]]; then
  echo "formal SpotifyLyrics database is unavailable" >&2
  exit 2
fi

TMP_DB="$TMP_DIR/SpotifyLyrics.sqlite3"
cp "$FORMAL_DB" "$TMP_DB"
...
swiftc -parse-as-library \
  ...
  "$ROOT_DIR/Tests/track_identity_v4_water_contract.swift" \
  -o "$TMP_DIR/track-identity-v4-water-contract"

"$TMP_DIR/track-identity-v4-water-contract" "$TMP_DB"
